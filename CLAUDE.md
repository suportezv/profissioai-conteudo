...
- **ElevenLabs**: chave `sk_...` (51 chars) na env var `ELEVENLABS_API_KEY` do cloud environment e no `.env` do video-use. Escopos confirmados: `text_to_speech`, `speech_to_text`, `sound_generation`, `voices_read`. **Ausentes**: `user_read` e `models_read` (sem `user_read` não dá para checar saldo de créditos antes de gerar lote).
  - Voz da Profissio.ai: **em decisão, com cinco candidatas já geradas e comparáveis** (21/set/2026), todas pt-BR, no artboard de locução do moodboard do case 03: `Eyspt3SYhZzXd1Jd3J8O` Bia (direta e assertiva, a recomendada), `ORgG8rwdAiMYRug8RJwR` Ana Alice, `wXwzHFLHnXex5h3JPBXA` Katiuscia, `wHnxjlY53t8X9Oi14Awz` Juliano, `4r3G9XKliGgVZLKMgjik` Lair. Parâmetros do teste: `eleven_multilingual_v2`, estabilidade 0,55, similaridade 0,75, estilo 0. **Não usar a voz clonada da Anaclaudia `XsU4z9JE7JPZzkVPg4GW`, que pertence ao ecossistema EITA.**
