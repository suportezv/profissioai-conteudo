...
- **Gemini (imagem)**: chave em `GEMINI_API_KEY`. **Texto funciona, imagem não.** Todo modelo de imagem devolve **429**, e o detalhe do erro fecha o diagnóstico: as violações são `quotaMetric: ...generate_content_free_tier_requests` com `quotaId: ...-FreeTier` e **`limit: 0`**. Ou seja, as chamadas continuam contadas no **tier gratuito**, onde imagem tem cota zero. Não é chave nem rede: `gemini-3.6-flash` responde texto normalmente com a mesma chave. **Adicionar meio de pagamento na conta não basta**; o faturamento precisa estar vinculado ao **projeto do Google Cloud daquela chave**. Enquanto o erro citar `-FreeTier`, o vínculo não pegou. Atenção: `gemini-2.5-flash` foi descontinuado para contas novas; o modelo de texto atual é `gemini-3.6-flash`.
